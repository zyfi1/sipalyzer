...
pub async fn reverse_lookup(ip: IpAddr) -> Option<String> {
    match tokio::time::timeout(Duration::from_secs(3), tokio::net::lookup_host(format!("{}:0", ip))).await
    {
        Ok(Ok(_)) => {
...
pub async fn batch_reverse_lookup(ips: &[IpAddr], concurrency: usize) -> Vec<(IpAddr, Option<String>)> {
