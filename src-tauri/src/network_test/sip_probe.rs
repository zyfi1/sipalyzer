...
fn compute_stats(
    ...
    let (avg_jitter, max_jitter) = if rtts.len() >= 2 {
        ...
        for i in 1..rtts.len() {
            jsamples.push((rtts[i] - rtts[i - 1]).abs());
        }
        let sum: f64 = jsamples.iter().sum();
        (
            sum / jsamples.len() as f64,
            jsamples.iter().cloned().fold(0.0f64, f64::max),
        )
    } else {
        (0.0, 0.0)
    };
...
async fn try_method_udp(
    ...
    for seq in 0..attempts {
        ...
        let request = match method {
            ProbeMethod::Options => {
                build_sip_options(host, port, local_ip, local_port, &call_id, seq + 1, &branch)
            }
            ProbeMethod::Register => {
                build_sip_register(host, port, local_ip, local_port, &call_id, seq + 1, &branch)
            }
            _ => unreachable!(),
        };
        if socket.send_to(request.as_bytes(), dest).await.is_ok() {
            let mut buf = [0u8; 4096];
            if tokio::time::timeout(timeout, socket.recv_from(&mut buf))
                .await
                .is_ok()
            {
                return true;
...
async fn try_method_tcp(dest: SocketAddr, timeout: Duration) -> bool {
    tokio::time::timeout(timeout, TcpStream::connect(dest))
        .await
        .is_ok()
}
...
async fn probe_sip_udp(
    ...
    let request = match method {
        ProbeMethod::Options => {
            build_sip_options(host, port, local_ip, local_port, call_id, cseq, &branch)
        }
        ProbeMethod::Register => {
            build_sip_register(host, port, local_ip, local_port, call_id, cseq, &branch)
        }
        _ => unreachable!(),
    };
    ...
    if config.method == ProbeMethod::Auto {
        ...
        let _ = window.emit(
            ...
            SipProbePacketEvent {
                ...
                running_sent: 0,
                running_received: 0,
                running_loss_pct: 0.0,
                running_avg_rtt_ms: 0.0,
                running_jitter_ms: 0.0,
            },
        );
...
async fn run_sip_udp_probes(
    ...
    let (avg, min, max, stddev, avg_jitter, max_jitter, loss) =
        compute_stats(&probe_rtts, sent, received);
