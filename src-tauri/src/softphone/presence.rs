...
pub fn publish_presence(
    ...
    if code == 401 || code == 407 {
        ...
        let auth_value = auth::build_digest_authorization("PUBLISH", &request_uri, username, password, &auth_challenge);
        ...
        if code2 < 200 || code2 >= 300 {
            return Err(format!("PUBLISH auth failed: {} {}", code2, resp2.status_text.as_deref().unwrap_or("")));
        }
        return Ok(resp2.get_header("SIP-ETag").cloned());
    }

    if code < 200 || code >= 300 {
        return Err(format!("PUBLISH failed: {} {}", code, resp.status_text.as_deref().unwrap_or("")));
    }
