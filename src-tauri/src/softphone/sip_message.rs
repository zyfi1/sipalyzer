...
pub fn send_message(
    ...
    let request_uri = if target.contains('@') {
        if target.starts_with("sip:") { target.to_string() } else { format!("sip:{}", target) }
    } else {
    ...
    req.add_header("Via", &format!("SIP/2.0/{} {}:{};rport;branch={}", via_transport, local_ip, local_port, branch));
    ...
    // Read response
    let response_bytes = transport.receive(Duration::from_secs(5)).map_err(|e| e.to_string())?;
    ...
    if code == 401 || code == 407 {
        ...
        let challenge_str = resp.get_header(auth_header_name).cloned().ok_or("No auth challenge")?;
        let default_realm = config.realm.as_deref().unwrap_or(&config.domain);
        let auth_challenge = auth::parse_auth_challenge(&challenge_str, default_realm).map_err(|e| e.to_string())?;
        let username = config.auth_username.as_ref().unwrap_or(&config.username);
        let password = &config.password;
        let auth_value = auth::build_digest_authorization("MESSAGE", &request_uri, username, password, &auth_challenge);

        let branch2 = format!("z9hG4bK{}", generate_tag());
        let mut auth_req = SipMessage::new_request("MESSAGE", &request_uri);
        auth_req.add_header("Via", &format!("SIP/2.0/{} {}:{};rport;branch={}", via_transport, local_ip, local_port, branch2));
        ...
        let resp2_bytes = transport.receive(Duration::from_secs(5)).map_err(|e| e.to_string())?;
        let resp2 = SipMessage::from_bytes(&resp2_bytes).map_err(|e| e.to_string())?;
        let code2 = resp2.status_code.unwrap_or(0);
        if code2 < 200 || code2 >= 300 {
            return Err(format!("MESSAGE auth failed: {} {}", code2, resp2.status_text.as_deref().unwrap_or("")));
        }
    } else if code < 200 || code >= 300 {
        return Err(format!("MESSAGE failed: {} {}", code, resp.status_text.as_deref().unwrap_or("")));
    }
...
pub fn handle_incoming_message(
    ...
    let content_type = msg.get_header("Content-Type").cloned().unwrap_or_else(|| "text/plain".to_string());
